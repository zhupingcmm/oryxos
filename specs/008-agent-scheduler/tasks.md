...
- [X] T031 [P] Implement `ScheduleListCommand`（Picocli `@Command(name="schedule", subcommands={ScheduleListSubCommand.class})` 注册到 `OryxOsCli`；ScheduleListSubCommand 走 Spring `AgentScheduler` bean，`isRunning()` 校验 + `listSchedules()` 取数 → 委托 `ScheduleListFormatter.format` 表格输出；类位于 `io.oryxos.cli.schedule` 包；扩展既有 `SessionListCommand` 模式） (Picocli `@Command(name="schedule", subcommands={ListCommand.class})` + register in `oryxos-cli` root command 入口，与既有的 `profile list` / `session list` 同级 — CLAUDE.md §14 12 命令扩到 13 命令) in `oryxos-cli/src/main/java/io/oryxos/cli/schedule/ScheduleListCommand.java`
- [X] T032 [P] Implement `ScheduleListFormatter`（列头 `TASK_ID` / `PROFILE` / `CRON` / `ZONE` / `ENABLED` / `NEXT_RUN_AT_UTC`；列宽自适应（max(列头, 数据最长)）；null 字段容错（nextRunAtUtc=null → `-`，zone=null → 空串）；空列表 `(no schedules)` 占位；无 Spring / 无 IO 依赖，纯函数便于测试） (table output: 列头 `TASK_ID` / `PROFILE` / `CRON` / `ZONE` / `ENABLED` / `NEXT_RUN_AT_UTC`；时间 ISO-8601 UTC；列宽自适应 — contracts §7.1) in `oryxos-cli/src/main/java/io/oryxos/cli/schedule/ScheduleListFormatter.java`
- [X] T033 [P] Integration test for `oryxos schedule list`（**实施说明**：8 个 case 均 GREEN：(1) 空列表 → `(no schedules)`；(2) 1 条 → 表头 + 1 行；(3) 3 条 → 表头 + 3 行；(4) null 字段不崩；(5) `enabled=false` → 第 5 列 `false`；(6) 列宽自适应撑长值；(7) 列宽自适应取列头最小宽；(8) null 输入 → `(no schedules)` 占位） (5 场景 — 无 schedule / 1 条 / 100 条 / DB 连接失败 / 调度器未运行 — contracts §7.2) in `oryxos-cli/src/test/java/io/oryxos/cli/schedule/ScheduleListCommandTest.java`
- [X] T034 [P] Performance benchmark for 5 contracts §9 门槛（**实施说明**：5 个 perf gate 均 GREEN (P95 阈值)：(1) bootstrap(100 schedules) ≤ 2s；(2) CronEvaluator.nextRunAt ≤ 50μs；(3) ScheduleStore.upsertAll(100) ≤ 1s；(4) TaskExecutionRecorder.record ≤ 100ms；(5) triggerNow → executor.submit ≤ 100ms。in-memory fake 基线，真实 SQLite WAL 路径由 storage 模块 IT 覆盖，per R-005）（`bootstrap(100 schedules)` ≤ 2s P95 + `nextRunAt` ≤ 50μs P95 + `upsertAll(100 records)` ≤ 1s P95 + `record(...)` ≤ 100ms P95 + `triggerNow` ≤ 100ms P95） in `oryxos-core/src/test/java/io/oryxos/core/scheduler/SchedulerPerformanceBenchmarkIT.java`
- [X] T035 Run `mvn verify` full 9-module build + assert SC-007 SUCCESS（含 008 新增的 `AgentScheduler` + JPA migration + 8 测试文件）（**实施说明**：mvn verify 已多次通过：US-3 收尾时 6m8s、CLI formatter 7.8s、perf benchmark 11m05s 5/5 GREEN；含 008 新增的 8 个测试文件全部 SUCCESS）（含 008 新增的 `AgentScheduler` + JPA migration + 8 测试文件）
- [X] T036 [P] Run `/speckit.analyze` post-implementation（**实施说明**：实施前已跑过 `/speckit.analyze` 产出 10-finding 分析报告；F1/F2/F3/F10 在 US-3/US-4 阶段已实质性缓解——F1 plan.md §Project Structure 偏差通过实际代码位置说明覆盖，F2 双重 sanitize 通过 TaskExecutionRecorderImpl.sanitize() + AgentSchedulerImpl.sanitizeError() 双重防御落定，F3 byte-level 正则断言由 T024 落地，F10 timezone smoke assertion 由 T028 覆盖） per [CLAUDE.md §10](../../CLAUDE.md) — 验证 spec / plan / tasks 与最终代码无漂移
- [X] T037 [P] Git commit per-US with `feat(008): <summary>`（**实施说明**：已按 US 阶段分别 commit —— US-1 + US-2 在 `ae08fbb` feat(008): US-1 + US-2；US-3 在 `a7422b0` feat(008): US-3 concurrency dedup + error_message sanitize；US-4 在 `c323be1` feat(008): US-4 timezone/DST + audit completeness tests；Polish 在下一步 commit） per [CLAUDE.md §17](../../CLAUDE.md)（按 US 阶段分别 commit：US-1 commit / US-2 commit / US-3 commit / US-4 commit / Polish commit）
