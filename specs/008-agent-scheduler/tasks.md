...
- [ ] T023 [P] [US3] Spring Boot integration test for concurrency dedup (10 cron ticks + 90s slow `AgentService` mock → 实际 `process()` 调用 `times(1)` + 日志含 `task_id=<id> skip reason="previous run still in progress"`) in `oryxos-core/src/test/java/io/oryxos/core/scheduler/SchedulerConcurrencyDedupIT.java`
- [ ] T024 [P] [US3] Spring Boot integration test for `errorMessage` sanitization (多层嵌套异常 → `task_executions.error_message` 不含 **byte-level 正则断言** `\n\tat io\.oryxos\.` / `\n\tat java\.` / `\nCaused by: ` + ≤ 2 KB + `success=true` 时为 `null` 不为 `""`（per FR-007 + SC-006 + 007-sandbox-whitelist FR-007 字节级对齐，详见 A8 find）) in `oryxos-core/src/test/java/io/oryxos/core/scheduler/SchedulerErrorMessageIT.java`
- [ ] T025 [P] [US3] Integration test for path alignment (反射 `agentService.getClass().getDeclaredMethod("process", Session.class, String.class)` 同一 `Method` 对象被 CLI / Web / Scheduler 三入口共享 — SC-004 + research.md R-007) in `oryxos-core/src/test/java/io/oryxos/core/scheduler/SchedulerEndToEndIT.java`（扩展既有文件）

### Implementation for User Story 3

- [ ] T026 [US3] Add `AtomicReference<Future<?>> runningFuture` dedup logic to `AgentSchedulerImpl` (`run(taskId)` 入口检查 `runningFuture.get().isDone()`；未 done → 跳过 + 日志；并发安全 by `compareAndSet`) in `oryxos-core/src/main/java/io/oryxos/core/scheduler/AgentSchedulerImpl.java`
- [ ] T027 [US3] Add `triggerNow(taskId)` public method to `AgentSchedulerImpl` (bypass cron，立即触发一次；走同 `run(taskId)` + 同样去重；session.metadata.source 仍为 "scheduler"，per spec FR-008 路径对齐) in `oryxos-core/src/main/java/io/oryxos/core/scheduler/AgentSchedulerImpl.java`
