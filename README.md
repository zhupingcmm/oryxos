...
| **Notify outbox (US-4 sub-capability)** | ✅ **Landed** | **[specs/004-notify-channel/](specs/004-notify-channel/)** | **`oryxos-tool/notify/`** |
...
>
> 004-notify-channel 已落地 — `notify(content, channel)` Tool 通过 webhook
> 出站推送，14 FR / 4 NFR / 8 SC 在 [specs/004-notify-channel/spec.md](specs/004-notify-channel/spec.md)
> 中固化；实现位于 [oryxos-tool/notify/](oryxos-tool/src/main/java/io/oryxos/tool/notify/)：
> `NotifyTool` 与 `WebhookNotifyAdapter` 加 9 个集成测试。覆盖：
> 单通道默认推送（P1）、多通道按名路由（P2）、Sandbox 域名白名单拦截
> 以及多通道并发广播（P3：`Profile.extra.broadcast=true` 且 LLM 不传
> channel 时并行发到全部 N 条通道，聚合语义支持全成功 / 部分成功 / 全失败）。
> 同时验证不重试（FR-011）、webhook URL 敏感 token 脱敏（FR-013）以及模块边界
> 检查（FR-014）。端到端冒烟：[scripts/notify-smoke.sh](scripts/notify-smoke.sh)
> （10 步 BSD sysexits）；FR-014 单测：[scripts/check-notify-module-boundary.sh](scripts/check-notify-module-boundary.sh)。
